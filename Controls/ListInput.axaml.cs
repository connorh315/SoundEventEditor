...
namespace SoundEventEditor
{
    ...
    public class ListInput : TemplatedControl
    {
        ...
        /// </summary>
        public static readonly StyledProperty<string> LabelProperty = AvaloniaProperty.Register<ListInput, string>(nameof(Label));
        ...
        /// </summary>
        public static readonly StyledProperty<ICommand> RemoveCommandProperty = AvaloniaProperty.Register<ListInput, ICommand>(nameof(RemoveCommand));
        ...
        {
            if (parameter is string item && DataContext is StringListOptionViewModel soundEventViewModel)
            {
                soundEventViewModel.RemoveItem(item);
            }
        }

        private void AddItemButton_Click(object sender, Avalonia.Interactivity.RoutedEventArgs e)
        {
            if (DataContext is not StringListOptionViewModel soundEventViewModel)
            {
                return;
            }

            soundEventViewModel.AddItem();
        }
    }
}